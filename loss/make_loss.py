...
def compute_itc(image_features, text_features, logit_scale=1):
    """
...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    #  elif cfg.DATALOADER.SAMPLER in ['softmax_triplet', 'id_triplet', 'img_triplet']:
        ...
        def loss_func(score, feat, target, target_cam, item):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    ...
                    if isinstance(feat, list):
                            ...
                            # TRI_LOSS = sum(TRI_LOSS) / len(TRI_LOSS)
                            # TRI_LOSS = 0.5 * TRI_LOSS + 0.5 * triplet(feat[0], target)[0]
                            TRI_LOSS = triplet(feat[0], target, normalize_feature=cfg.SOLVER.TRP_L2)[0]
