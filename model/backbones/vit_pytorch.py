...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=False):
        ...
        self.cls_token_dis_rgb = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.cls_token_dis_ir = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        self.pos_embed_f = nn.Parameter(torch.zeros(1, num_patches + 2, embed_dim))
    ...
    def forward_features(self, x, camera_id, view_id, mode):
        B = x.shape[0]
        ...
        if self.training:
            ...
            rgb_dis_cls_tokens = self.cls_token_dis_rgb.expand(int(B/2.0), -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            ir_dis_cls_tokens = self.cls_token_dis_ir.expand(int(B/2.0), -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            x_rgb = torch.cat((rgb_dis_cls_tokens, x[view_id==1]), dim=1)
            x_ir = torch.cat((ir_dis_cls_tokens, x[view_id==0]), dim=1)
            ...
            cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            if mode == 1:
                rgb_dis_cls_tokens = self.cls_token_dis_rgb.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
                # x = torch.cat((cls_tokens, rgb_dis_cls_tokens, x), dim=1)
                x = torch.cat((rgb_dis_cls_tokens, cls_tokens, x), dim=1)

            else:
                ir_dis_cls_tokens = self.cls_token_dis_ir.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
                # x = torch.cat((cls_tokens, ir_dis_cls_tokens, x), dim=1)
                x = torch.cat((ir_dis_cls_tokens, cls_tokens, x), dim=1)
        ...
        if self.cam_num > 0 and self.view_num > 0:
            x = x + self.pos_embed_f + self.sie_xishu * self.sie_embed[camera_id * self.view_num + view_id]
        elif self.cam_num > 0:
            x = x + self.pos_embed_f + self.sie_xishu * self.sie_embed[camera_id]
        elif self.view_num > 0:
            x = x + self.pos_embed_f + self.sie_xishu * self.sie_embed[view_id]
        else:
    ...
    def forward(self, x, cam_label=None, view_label=None, mode=None):
        x, xx = self.forward_features(x, cam_label, view_label, mode)
        return x, xx
